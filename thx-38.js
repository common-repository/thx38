...
if ( themes.isInstall ) {
	...
	// These are submitted by the themes.data.browse['sections'] property
	themes.view.Browser = wp.Backbone.View.extend({
		...
		render: function() {
			...
			self.$el.append( '<h3 class="theme-section">' + themes.data.browse.sections[ self.options.section ] + '</h3><div class="themes theme-browser ' + self.options.section + '"></div>' );
			...
			self.collection.each( function( theme ) {
				...
				// Render the views...
				self.theme.render();
				self.$el.find( '.themes' ).append( self.theme.el );
	...
	// Overwrite main initializer
	themes.Run.init = function() {
		// Loop through the different theme sections
		// and sets up each one of them to be rendered
		for ( var section in themes.data.browse.sections ) {

			// Create a new collection with the proper theme data
			// for each section
			this.themes = new themes.Collection( themes.data.browse.publicThemes[ section ].themes );

			// Set up the view
			// Passes the 'section' as an option
			this.view = new themes.view.Install({
				el: $( '#appearance' ),
				collection: this.themes,
				section: section
			});
